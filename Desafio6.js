...
while(adicionarMais != "não"){
    if(frutas.length === 0 && acougue.length === 0 && doces.length === 0 && congelados.length === 0){
    ...
    if(adicionarMais === "sim"){
        ...
        categoria = prompt("Em qual categoria essa comida se encaixa: 'frutas', 'acougue', 'doces' ou 'congelados'?");
        if(categoria === 'frutas'){
            frutas.push(comida);
	...
	if(frutas.length === 0 && laticinios.length === 0 && doces.length === 0 && congelados.length === 0){  //se a lista estiver vazia (tratamento de bug, caso a pessoa digite "remover" mesmo quando forem exibidas apenas as opções "sim" e "não"
		...
		if(frutas.indexOf(remover) != -1){
			...
			laticinios.splice(acougue.indexOf(remover), 1);
			alert(`O item ${remover} foi removido com sucesso!`)
